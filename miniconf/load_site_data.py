...
def generate_paper_events(site_data: Dict[str, Any]):
    ...
    for uid, session in site_data["paper_sessions"].items():
        ...
        event = {
            "title": f"<b>{session['name']}</b><br>{session['long_name']}",
            "start": start,
            "end": end,
            "location": "",
...
def build_papers(
    ...
    for session_id, session in paper_sessions.items():
        for paper_id in session["papers"]:
            assert paper_id not in paper_id_to_link, f"{paper_id} has been seen twice!"
            # if paper_id in paper_id_to_link:
            #     continue
            if session_id.startswith("z"):
                ...
                paper_id_to_link[
                    paper_id
                ] = "https://www.virtualchair.net/events/eacl2021"
